# Forge: The Native AI Coding Agent

## Product Vision
Forge is a local-first, native desktop application serving as a serious AI coding agent for Windows and Linux. It is not an opaque chatbot or a simple LLM wrapper. Forge is a powerful, repository-aware engineer capable of autonomous planning, architectural design, implementation, and deployment execution safely under user supervision.
...
## Core User Stories
1. **As a developer**, I want to provide a high-level task to Forge so that it can submit an architectural implementation plan before diving into code.
...
3. **As a web developer**, I want Forge to spawn a headless browser, navigate to my dev server, take screenshots, and auto-correct visual bugs based on its observations.
...
- `forge-tools` capabilities: basic file read/write, terminal execution.
- Typed approval request flows in UI.

### v1.1 Scope
- Introduce `forge-browser` CDP hook for basic site snapshots and interaction.
- AST semantic search indexing integration.

### v2 Scope
- Long-term memory integration with SQLite (`forge-memory`).
- Full UI state persistence across app reboots.
- Complete ACP-style text editor plugin system.

## Success Criteria
- [ ] Users can bootstrap a full feature with Forge end-to-end without touching a terminal directly.
